...
export default function HeadlineCrumble({ 
  ...
  return (
    <motion.span 
      className={`cursor-pointer select-none inline-block ${className}`}
      variants={container}
      initial="hidden"
      animate="visible"
    >
      {letters.map((letter, index) => {
        ...
        const bottomOfScreen = (typeof window !== 'undefined' ? window.innerHeight : 800) - 100;
          ...
          <motion.span
            ...
            className={`inline-block transition-all duration-200 ${
              !letterState.isFalling 
                ? 'hover:cursor-pointer hover:scale-110 hover:text-red-500' 
                : 'cursor-default'
            }`}
            style={{ 
